...
{
	a600_setup_page_tables(bl1_tzram_layout.total_base,
...
void bl1_platform_setup(void)
{
	...
	rc = a600_vc_hardware_get_board_revision(&rev);
	...
	if (rc == 0) {
		...
		default:
			model = "Unknown";
			info = "(Unknown)";
			ERROR("a600: Unknown board revision 0x%08x\n", rev);
			break;
		}

		NOTICE("a600: Detected: %s %s [0x%08x]\n", model, info, rev);
	} else {
		ERROR("a600: Unable to detect board revision\n");
	}
