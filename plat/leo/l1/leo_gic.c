 ...
 * interrupts.
 *****************************************************************************/
static const interrupt_prop_t leo_interrupt_props[] = {
...
static gicv2_driver_data_t platform_gic_data = {
	.interrupt_props = leo_interrupt_props,
	.interrupt_props_num = ARRAY_SIZE(leo_interrupt_props),
	.target_masks = target_mask_array,
	.target_masks_num = ARRAY_SIZE(target_mask_array),
};

static struct leo_gic_instance leo_gic;
...
void leo_gic_init(void)
{
	...
	leo_gic.cells = fdt32_to_cpu(*cuint);

	leo_gic.phandle_node = fdt_get_phandle(fdt, node);
	if (leo_gic.phandle_node == 0U) {
		panic();
	}
